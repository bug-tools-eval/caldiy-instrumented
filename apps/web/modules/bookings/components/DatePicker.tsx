...
const useMoveToNextMonthOnNoAvailability = ({
  ...
  // nonEmptyScheduleDays are "YYYY-MM-DD" strings, so a string prefix match
  // avoids constructing a timezone-aware Dayjs and calling .isSame per element.
  const browsingMonthPrefix = browsingDate.format("YYYY-MM");
  const nonEmptyScheduleDaysInBrowsingMonth = nonEmptyScheduleDays.filter((date) =>
    date.startsWith(browsingMonthPrefix)
  );
